...
class ContactsGateway {
	...
	public function selectUpdateSuccess($id) {
		$ini_array = $this->getEventsCutOff();
		$event=$ini_array[0]->id;
		$dbId = mysql_real_escape_string($id);
		$dbres = mysql_query("SELECT * FROM candidates_$event WHERE id=$dbId and event=$event");
	...
	public function editById($search) {
		$ini_array = $this->getEventsCutOff();
		$event=$ini_array[0]->id;
	...
	public function updateContact($id,$fname,$lname,$email,$mobile,$dob,$gender,$college,$qualification,$stream,$ssc,$inter,$degree,$yearPassed,$currentCompany,$yearsOfExp,$source)	{
		...
		if($id=="" || $id==NULL){
			...
			mysql_query("UPDATE candidates_$event SET fname=$dbfname,lname=$dblname,email=$dbemail,mobile=$dbmobile,dob=$dbdob,gender=$dbgender,college=$dbcollege,qualification=$dbqualification,stream=$dbstream,yearPassed=$dbyearPassed,ssc=$dbssc,inter=$dbinter,degree=$dbdegree,currentCompany=$dbcurrentCompany,yearsOfExperience=$dbyearsOfExp,source=$dbsource WHERE regId=$id");
		}
        return mysql_insert_id();
    }
  ...
  if(mysql_num_rows($query)!=0)
			{
				...
				echo "email id exists already";
exit;
			}
			else{
	if(@mysql_query($res0))
	{
		$this->sendEmailDetails($dbEmail,$dbStatus);		
		}		*/
					return $candId;
	...
	public function sendEmailDetails($tId,$status){
		$ini_array = $this->getEventsCutOff();
		...
		if($eventNotify == 1){
			...
			$res1 = mysql_query("select id,regId,email,fname,lname from candidates_$event where email=$tId");		
			$result1 = array();		
