...
class ContactsController {
	...
	public function doLogout1() {
		...
		session_destroy();
		$xyz = $this->contactsService->getCount();
        include 'application/view/loginstaff.php';
	}
    ...
    public function saveContact() {
        ...
        if ( isset($_POST['form-submitted']) ) {
			...
			$ssc      = isset($_POST['ssc'])?   $_POST['ssc'] :NULL; // textbox
			$inter      = isset($_POST['inter'])?   $_POST['inter'] :NULL; // radio button
	...
	public function updateSuccess() {
        include 'application/view/updateSuccess.php';
    }
 ...
 $fname = '';
        ...
        if ( isset($_POST['form-submitted']) ) {
			$regId2 = isset($_POST['l_id'])? $_POST['l_id'] :NULL;
		    $fname       = isset($_POST['fname']) ?   $_POST['fname']  :NULL;
			$lname       = isset($_POST['lname']) ?   $_POST['lname']  :NULL;
            $email      = isset($_POST['l_email'])?   $_POST['l_email'] :NULL;
		    $mobile      = isset($_POST['mobile'])?   $_POST['mobile'] :NULL;
			...
			$source      = isset($_POST['source'])?   $_POST['source'] :NULL;
            try {
                $res = $this->contactsService->updateContact($regId2,$fname,$lname,  $email,$mobile,$dob,$gender, $college, $qualification, $stream, $ssc, $inter, $degree, $yearPassed, $currentCompany,$yearsOfExp, $source);
				print_r($res);
				  $this->redirect('index.php?op=updateSuccess');
                return $res;
            } catch (ValidationException $e) {
                $errors = $e->getErrors();
            }
		...
		try{
			$edit = $this->contactsService->getEdit($regId);
		}catch (ValidationException $e) {
			$errors = $e->getErrors();
		}
		$xyz = $this->contactsService->getCount();
		...
		include 'application/view/edit.php';
